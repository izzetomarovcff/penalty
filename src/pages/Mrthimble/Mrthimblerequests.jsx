...
function Mrthimblerequests() {
    const [data, setData] = useState(null)
    const [accepting,setAccepting] = useState(false)
    const [acceptingid,setAcceptingid] = useState(null)
    const put_user_data = async (user_id, data) => {
        try {
            await fetch(`https://moccha77-fbe81-default-rtdb.firebaseio.com/mrthimbleusers/${user_id}.json`, {
                method: 'PUT',
                headers: {
    ...
    const get_user_data = async (user_id) => {
        try {
            let response = await fetch(`https://moccha77-fbe81-default-rtdb.firebaseio.com/mrthimbleusers/${user_id}.json`)
...
export default Mrthimblerequests