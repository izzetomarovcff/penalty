...
function Penaltyrequests() {
    const [data, setData] = useState(null)
    const [accepting,setAccepting] = useState(false)
    const [acceptingid,setAcceptingid] = useState(null)
    const put_user_data = async (user_id, data) => {
        try {
            await fetch(`https://moccha77-fbe81-default-rtdb.firebaseio.com/penaltyusers/${user_id}.json`, {
                method: 'PUT',
                headers: {
    ...
    const get_user_data = async (user_id) => {
        try {
            let response = await fetch(`https://moccha77-fbe81-default-rtdb.firebaseio.com/penaltyusers/${user_id}.json`)
    ...
    const fetchData = async () => {
        try {
            const response = await fetch("https://moccha77-fbe81-default-rtdb.firebaseio.com/penaltyusers.json")
            ...
            {data ? (
                <div className="reqdiv rounded shadow border">
                    {Object.entries(data).map(([index, user]) => {
                        if(!user.accepted){
                            return (
                                <div className='requestdivm shadow m-3 p-2' key={index}>
