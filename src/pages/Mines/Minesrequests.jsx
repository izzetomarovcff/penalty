...
function Minesrequests() {
    const [data, setData] = useState(null)
    const [accepting,setAccepting] = useState(false)
    const [acceptingid,setAcceptingid] = useState(null)
    const put_user_data = async (user_id, data) => {
        try {
            await fetch(`https://moccha77-fbe81-default-rtdb.firebaseio.com/minesusers/${user_id}.json`, {
                method: 'PUT',
                headers: {
    ...
    const get_user_data = async (user_id) => {
        try {
            let response = await fetch(`https://moccha77-fbe81-default-rtdb.firebaseio.com/minesusers/${user_id}.json`)
    ...
    const fetchData = async () => {
        try {
            const response = await fetch("https://moccha77-fbe81-default-rtdb.firebaseio.com/minesusers.json")
    ...
    return (
        <div className='requestpage'>
            <h2 className='mt-2'>Mines İstəklər</h2>
            {data ? (
